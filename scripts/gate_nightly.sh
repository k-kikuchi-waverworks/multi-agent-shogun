...
if [ "$rc1" -ne 0 ] || [ "$rc2" -ne 0 ] || [ "$hook_rc" -ne 0 ]; then
    ...
    msg="【gate_nightly警告】沈黙落とし穴gate非PASS=gate-1(commit捕捉)=$(verdict "$rc1")/gate-2(変異台帳)=$(verdict "$rc2")。${hooknote}所見=${detail} 処方=docs/content/ops/cmd_1352_silent_pitfall_gates.md を見て名指しされた項目を是正し、bash scripts/gate_artifact_capture.sh --all と python3 scripts/gate_mutation_replay.py の再走で緑を確認せよ。"
...
echo "── [gate_nightly] 終了 gate-1=$(verdict "$rc1") gate-2=$(verdict "$rc2") hook=$([ "$hook_rc" -eq 0 ] && echo OK || echo MISSING) ──"
if [ "$rc1" -eq 1 ] || [ "$rc2" -eq 1 ]; then exit 1; fi
if [ "$rc1" -ne 0 ] || [ "$rc2" -ne 0 ] || [ "$hook_rc" -ne 0 ]; then exit 2; fi
exit 0
