...
| 2 | `git commit -- <パス列>` で撃つ（CLAUDE.md 規律 5） | index に何が積まれていても、指した path だけが commit される |
| 3 | commit の**後**に `git show --stat HEAD` で自分の物だけか検める | 前では防げない。後なら必ず捕える |
...
| 盤面 | 使う手 |
|---|---|
| 己しか触っておらぬ file | `git commit -- <パス列>` (手2 のまま) |
| ★同じ file を他者も触っておる★ | ★**hunk を選んで index へ載せてから commit**★。**path 指定に頼るな** |
| どちらの場合も | commit の**後**に `git show --stat HEAD` で己の物だけか検める (手3・不変) |
