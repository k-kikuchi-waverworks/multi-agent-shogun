...
# Function: emit_canonical_role_fenced
# Description: Emits instructions/{role}.md for outputs that already carry their own
#   YAML front matter (OpenCode agent definitions). The canonical file's front matter
#   is wrapped in a ```yaml fence so the reader still gets forbidden_actions etc.
#   without a second front-matter block confusing the parser. CR is stripped.
# Arguments: $1 = role name
# Returns: 0 (prints nothing if the canonical file is missing)
emit_canonical_role_fenced() {
    local role="$1"
    local src="$ROOT_DIR/instructions/${role}.md"

    [ -f "$src" ] || return 0

    tr -d '\r' < "$src" | awk '
        NR == 1 && $0 == "---" { print "```yaml"; fence = 1; next }
        fence == 1 && $0 == "---" { print "```"; fence = 0; next }
        { print }
    '
}
...
build_instruction_file() {
    ...
    # Emit the canonical role file as-is (YAML front matter + body).
    #
    # cmd_1451 案B: instructions/{role}.md is the single source of truth for role
    # content. The old form took only the front matter from here and the body from
    # instructions/roles/{role}_role.md, so 190 lines that live only in the canonical
    # file never reached any non-Claude CLI (karo: 84% of its own lines).
    #
    # CR is stripped here on purpose. The old awk matched /^---$/, which never matches
    # "---\r", so for the three CRLF files (shogun / ashigaru / gunshi) the front matter
    # came out EMPTY — and nothing in the build said so.
    if [ -f "$original_file" ]; then
        tr -d '\r' < "$original_file" > "$output_path"
        echo "" >> "$output_path"
    else
...
mode: primary
# Auto-generated by build_instructions.sh — do not edit manually.
# Source: instructions/${role}.md + instructions/common/* + instructions/cli_specific/opencode_tools.md
...
FRONTMATTER
        ...
        {
            emit_canonical_role_fenced "$role"
