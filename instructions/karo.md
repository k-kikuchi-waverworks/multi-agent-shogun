...
★`target_path` は【其の agent 自身が書く file】でなければならぬ★。他人が書く共有 file (例: `queue/inbox/…`) を
`target_path` に置けば、★**他人の筆で己の沈黙が隠れる**★ = **永久免除に近い偽の緑**になる。

⇒ ★★`target_path` の mtime もまた二義である★★ (「己が働いた」と「他人が書いた」が同じ顔で返る)。

**規**:
- 置くのは **agent 自身が書く成果物** の路のみ。共有 file・他人の書く file は置くな。
