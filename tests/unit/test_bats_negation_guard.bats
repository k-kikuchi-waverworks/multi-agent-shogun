...
@test "T-NEG-001: the repository currently has zero toothless negations" {
    ...
    # ★下限を持たせる (2026-07-27 刈込みの代償)★:
    #   門が歩く木を PRUNE_DIRS で刈った = ★名で刈るゆえ、刈り過ぎても【速くなった】ようにしか見えぬ★。
    #   ★数が減れば、其れは【速くなった】のでなく【見なくなった】である★ (家老 03:22) —
    #   「1 file 以上」では 47→3 の崩落を緑で通す。刈込み時点の実測 = ★47 file★ゆえ
    #   ★30 を下限★に据える (悉皆の pin ではない = 正当な削除で脆く赤にせぬため余裕を取った)。
    local n
    n="$(echo "$output" | sed -n 's/^\[gate_bats_negation\] 走査 \([0-9]*\) file.*/\1/p')"
    [ -n "$n" ]
    if [ "$n" -lt 30 ]; then
        echo "★走査 file 数が $n 件まで落ちた (刈込み時点の実測 = 47)★" >&2
        echo "★見落としを疑え = PRUNE_DIRS が刈り過ぎておらぬか★" >&2
        return 1
    fi
}
