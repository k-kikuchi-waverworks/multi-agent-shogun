...
workflow:
  ...
  - step: 9
    action: receive_wakeup
    from: gunshi  # cmd_645 (2026-05-10) 後は gunshi_a / gunshi_b
    via: inbox
    note: "Gunshi reports QC results. Ashigaru no longer reports directly to Karo."
  - step: 10
    action: scan_all_reports
    target: "queue/reports/ashigaru*_report.yaml + queue/reports/gunshi_report.yaml + queue/reports/gunshi_a_report.yaml + queue/reports/gunshi_b_report.yaml"
    note: "Scan ALL reports (ashigaru + gunshi/gunshi_a/gunshi_b). cmd_645 (2026-05-10) gunshi 2-instance arch — gunshi retained for backward compat. Communication loss safety net."
  - step: 11
    action: update_dashboard
    target: dashboard.md
    section: "戦果"
...
files:
  ...
  # cmd_645 (2026-05-10): gunshi → gunshi_a + gunshi_b 分割。gunshi_task/gunshi_report は backward compat retain。
  gunshi_task: queue/tasks/gunshi.yaml         # deprecated (backward compat)
  gunshi_a_task: queue/tasks/gunshi_a.yaml     # ML/AI/データ系
  gunshi_b_task: queue/tasks/gunshi_b.yaml     # infra/dev/規律系
  report_pattern: "queue/reports/ashigaru{N}_report.yaml"
  gunshi_report: queue/reports/gunshi_report.yaml         # deprecated (backward compat)
  gunshi_a_report: queue/reports/gunshi_a_report.yaml
  gunshi_b_report: queue/reports/gunshi_b_report.yaml
...
panes:
  ...
  gunshi: { pane: "multiagent:0.8" }                                                # deprecated (backward compat, == gunshi_a pane)
  gunshi_a: { pane: "multiagent:0.8", domain: "ML/AI/データ系" }
  gunshi_b: { pane: "multiagent:0.9", domain: "infra/dev/規律系" }                  # 殿手動 trigger 必須 (pane 0.9 起動 + claude 起動)
...
## 軍師 dispatch 振り分け規律 (cmd_645 起源)

cmd_645 (2026-05-10) で軍師 2 体制が確立された。家老は cmd の主領域で軍師 A/B に振り分ける。殿明示承認 msg_123656 (12:36 「いいよ」)、軍師 plan `plans/cmd_645_2_gunshi_architecture.md` §3 単一案 A 採択。

### 領域 lookup table

| 主領域 | 軍師 | pane | dispatch keyword (例) |
|--------|------|------|----------------------|
| ML/AI/データ | gunshi_a | multiagent:agents.8 | LoRA, fine-tune, RAG, TTS, vLLM, prompt eng, Vedal, embedding, axolotl, dataset, koi_v\d+, 学習, 推論 |
| infra/dev/規律 | gunshi_b | multiagent:agents.9 | ai-automate-engine, dev:all, npm, PowerShell, regression, smoke, E2E, Discord, OBS, dispatch template, Spot QC, commit hash verification, audit, 規律 |

### 振り分け手順

1. cmd タイトル + description から主領域 keyword 抽出
2. lookup table で軍師 A/B 判定
3. 判定不能時 (overlap が 50/50) → 殿に判断仰ぐ (Tier 1 ntfy or dashboard)
4. 主軍師に dispatch、副領域は主軍師の plan 内で別 cmd 候補として整理

### 衝突調停手順

- 同時 dispatch 禁止 (両軍師に同 cmd を同時 dispatch しない、RACE-001 相当)
- 主軍師経由で副軍師領域を切り出し
- 例: cmd_641 (npm dev:all) → 軍師 B 主、ML 系副 (なし) → 軍師 B のみ dispatch
- 例: cmd_643 (Vedal fine-tune) → 軍師 A 主、Modal config 副 (infra) → 軍師 A のみ dispatch、Modal config は ash 実装で完結
- 例: cmd_611 (TTS SBV2 学習) → 軍師 A 主、デプロイ副 (infra) → 軍師 A のみ dispatch

### task YAML path 振り分け

- 軍師 A 向け: `queue/tasks/gunshi_a.yaml`
- 軍師 B 向け: `queue/tasks/gunshi_b.yaml`
- inbox 通知:
  - `bash scripts/inbox_write.sh gunshi_a "..." task_assigned karo`
  - `bash scripts/inbox_write.sh gunshi_b "..." task_assigned karo`
- ★既存 `queue/tasks/gunshi.yaml`★ は backward compat retain (cmd_645 完遂前 or 過渡期 fallback)、新規 dispatch では使用しない。

### 相互 spot QC dispatch

- 軍師 A 起草 plan の spot QC → 軍師 B に dispatch (`gunshi_b.yaml` 経由)
- 軍師 B 起草 plan の spot QC → 軍師 A に dispatch (`gunshi_a.yaml` 経由)
- 規律 cmd (cmd_640/639/645 等) は軍師 B 担当 (規律領域)
- 自軍師領域 ash 完遂報告 (`report_received` inbox) の spot QC は同一軍師に dispatch (実装詳細知識必要)

### Wake = Full Scan 拡張 (cmd_645)

`§ "Wake = Full Scan" Pattern` (line 344+) 適用時、scan 対象を拡張:

```
queue/reports/ashigaru*_report.yaml
+ queue/reports/gunshi_report.yaml         (deprecated, backward compat)
+ queue/reports/gunshi_a_report.yaml
+ queue/reports/gunshi_b_report.yaml
```

`§ Stall Watchdog` (line 644+) inbox 検査も同様に gunshi_a/gunshi_b 両 inbox 走査必須:

```
queue/inbox/gunshi.yaml         (deprecated, backward compat)
...
cmd_645 ash 実装で settings.yaml + scripts + queue/ 構造は分割済だが、★tmux pane 0.9 起動 + 軍師 B claude 起動は殿手動 trigger★ (Phase 6 完遂後)。pane 0.9 未起動時の挙動:

- 軍師 A (pane 0.8) は通常運用継続
- 軍師 B 担当 cmd 発生時 → 家老が殿に Tier 1 ntfy で「pane 0.9 起動 + claude 起動」依頼
- 殿手動完遂まで `queue/tasks/gunshi_b.yaml` は assigned 状態で保留 (軍師 B が起動次第 Session Start で読込)
- 殿戻り後 `tmux set-option -p -t multiagent:agents.9 @agent_id "gunshi_b"` 設定確認

### 殿手番 ntfy 文言例 (Tier 1)

```
[cmd_645] 軍師 2 体制起動依頼。tmux pane 0.9 を multiagent session に追加 (split-window) + claude 起動 + @agent_id="gunshi_b" 設定御願い候。完遂後家老まで返信されたし。
```

### 同時 dispatch 禁止 (RACE-001)

- 同一 cmd を gunshi_a + gunshi_b 両方に同時 dispatch する状態を絶対禁止
- 衝突時は主領域で振り分け、判定不能時は殿判断仰ぐ
- ★軍師 A の起草 plan を軍師 B が QC する flow は ★同一 cmd 内の異なる task★ ゆえ並列禁止対象外★ (順次 dispatch)
