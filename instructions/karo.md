...
★★家老は 05:37 に「路だけ書け」と己で規を据え、其のとおり書いて、なお後者を踏んだ★★
(六号の `target_path` に前任 cmd_1382 の成果物 `config/mutation_registry.yaml` を残した。
本任の成果物は `scripts/gate_nightly.sh` であった。05:50 に再是正)。
⇒ ★★「路だけ書け」では足りぬ。任が替われば target_path も替わる★★。

⇒ ★★`target_path` の mtime は三義である★★ (「己が今の任で働いた」「他人が書いた」「己が前の任で書いた」が同じ顔で返る)。

**規**:
- 置くのは ★**今の任で** agent 自身が書く成果物★ の路のみ。共有 file・他人の書く file・**前任の成果物**は置くな。
- ★**task_id / parent_cmd を替える時は、target_path も同じ turn で見直せ**★ (三点は一組である)。
