...
try:
    ...
    if len(data["messages"]) > 50:
        ...
        # 未読は全部 残し、既読は新しい方から一定数だけ残す。
        #
        # ★30 から 15 へ下げた (cmd_1467・2026-07-28)★
        # 下げた理由は「通数」と「読める量」が別物だからである。この切り詰めは通数
        # (上限 50) で引くが、読む口が切るのは token (上限 25,000) である。ゆえに
        # 切り詰めが走り切った直後でも、便が読めないままになる形が現に在った。
        #
        # 実測 (2026-07-28 15:11:58・走査器 plans/cmd_1467_inbox_trigger_scan.py) =
        # 便 9 本を測ると、上限に収まるのは 14〜19 通であった。残す数 30 は、その約2倍。
        # 引き金の 50 通に至っては約3倍で、9 本とも現に上限を越えていた。
        # 15 なら 9 本とも、換算の厳しい側でも上限の内に入る。
        #
        # ★「15 にすれば必ず収まる」とは書けない★ = 上の数は byte から幅つきで換算した
        # 物で、token そのものを測ってはいない。現に切れるかは、その file を現に開くまで
        # 確定しない。言えるのは「測った 9 本は、換算の幅の厳しい側でも内へ入る」までである。
        #
        # 引き金 (50) は動かしていない。替えたのはここ1箇所だけである。
        KEEP_READ = 15
        keep_read = read[-KEEP_READ:]
