...
# ═══════════════════════════════════════════════════════════════════
# cmd_1385: ★走った刻を1行だけ残す★
# ───────────────────────────────────────────────────────────────────
# 2026-07-26 に「本日 番人は何本 警報を上げたか」を数えようとして数えられなんだ。
# log は 16,000 行あって ★1 行も時刻を持たず★、どの行が何時のものか復元するには
# IDLE_MIN の 3 分刻みを人が手で辿るしか無かった (実際にそうした)。
# ⇒ 1 run 1 行の見出しを置く。これだけで全行が「直前の見出しの run のもの」として
#   日付を持つ。log を溢れさせぬため見出しは 1 行に限る (cmd_1379 の枷と同じ流儀 —
#   毎 tick 書き散らせば読まれぬ = 黙っておるのと同じ)。
# ═══════════════════════════════════════════════════════════════════
echo "[idle_revive] ===== scan $(date +%Y-%m-%dT%H:%M:%S%z) pid=$$ =====" >&2
