...
# 今夜の規（6条・2026-07-28 未明の実戦から）

出所は軍師二号の稿 **2 本**で、いずれも検分を通り、現物に接地しています。
...
**合わせて 6 条・実例 6 件です。**（六号が 07:0x に己で数え直しました。条B = 数は写さず己で数える）
...
**出所を確かめるための条が、出所の誤りを抱えたまま copilot-instructions.md へ入るところでした。**
...
- `AGENTS.md:289` は **「Codex CLI rejects Write/Edit on unread files.」** と刷っていました。
  **Codex では一度も測られていません。** 正本には、既定 CLI について測った一文が在っただけです。
- **より重い方**: `AGENTS.md:106` は
  **「*Codex CLI users: this file is also auto-loaded via Codex CLI's memory feature.*」**
  と刷っていました。**同じ repo の `instructions/cli_specific/codex_tools.md:81` は
  「Codex does not have a built-in persistent memory system」と書いています。**
...
- **数はまだ閉じていません。** 一号は「正本に他へ何件 CLI 固有の記述が在るかは測っておらぬ」と
  名乗りました。cmd_1455 で 2 行を直しましたが、**残る 3 行
  （`common/protocol.md:146` / `common/task_flow.md:145` / `karo.md:559`）は手つかずです。**
