...
- gate-2 の `★同一の綴り置換が N 箇所で発火 (申告は M 箇所)★` (cmd_1382) … anchor が非一意である。
  ...
  **`塊の出所: <file>×<数>` も併せて出る** (cmd_1387・2026-07-27 に足した) —
  **行の塊の側が鳴った時、綴りの内訳は常に空である**ゆえ、これが唯一の手掛かりになる。
  ★何故 足したか★ — 綴りの内訳は**同一の (old→new) が 2 回以上出た時にしか埋まらぬ**。
  ゆえに `綴り 1 / 行の塊 2` の盤面では**内訳が空で出る** = **鳴っても持ち主が動けぬ**。
  2026-07-27 18:40 に現に起きた: 鳴らされた牙の持ち主へ内訳を渡せず、
  持ち主は静かな盤面で 3 回 撃ち直して 3 回とも PASS を見るしか無かった
  (**その盤面では再現せぬ**形であったゆえ)。**file 名さえ出ておれば、
  「己の変異が書かぬ file の塊である」とその場で判った**。
  ⇒ **読み方**: 出た file が**自分の mutate が書かぬ file** なら、着弾ではなく
  **他人が同じ木へ書いた行**を疑え (写しの取り方が原因の族。cmd_1387 で塞いだ)。
...
### 幽霊の **出所を割る** + 物差しの是正 (cmd_1408 / 2026-07-27・足軽三号)

**なぜ要ったか**: 2026-07-27 06:30 の朝の門で **ID言及 133 件中 幽霊 63 件** が鳴った。
家老は「文書が ID を引いたゆえ門の射程の問題」と読んだが**外れ** (文書の引用は一件も無し)。
実測で割ると **三つの全く違う物**が「幽霊」という一語に同居しておった:

| 別 | 意味 | 手番 |
|----|------|------|
| **A** | 本 repo の台帳には無いが**別台帳に実在** (backend/app/web/engine) | 登録では消えぬ。木を跨ぐ引用と判る書き方か、其の木の gate で見る |
| **C** | **真に未登録** = 何処の台帳にも無い | 所有者が登録する (**推測で埋めるな**) |
| **C?** | 別台帳が読めなんだゆえ **判別不能** | 先に照合先を直す (**「読めぬ」を「無い」へ倒さぬ**) |
...
**併せて物差しを是正した (己の捨てた物が門の中で生きておった)**: 旧 `REGISTRY_ID_RE` は
足軽三号が cmd_1386 (`scripts/ledger_id_census.py`) で**壊れておると証して捨てた綴りその物**
であった。
（★この `scripts/ledger_id_census.py` は cmd_1479 第4束で撤去した★ = 呼び手が1本も無かったため。
現物は `git show 88aa167:scripts/ledger_id_census.py`。
**是正そのものは生きている** = 門の側の `REGISTRY_ID_RE` は今も正しい綴りで、撤去の前に
撤去する側の `ID_RE` と機械で突き合わせて**文字まで同一**であることを確かめてある。
**変わったのは「突き合わせる相手が repo に無くなった」ことだけ**である。）破れは二つ — **枝を 1 つしか読まぬ** (`…-R3-M1〜M5` が `…-R3` へ潰れる) /
**先読みが無い** (`XMUT-…` の綴りの途中から拾う)。前者は**偽陰性**を作る =
台帳に `…-R3` が在れば、実在せぬ `…-R3-M9` の申告を門は `…-R3` と読んで**黙る**。
**是正の結果、幽霊は 63 → 64 件へ【増えた】** (07:57 実測・shogun 木) =
**数を良くする動きではない**ことが数で示せる形。

**捨てた綴りは消さず、影を数える器として残す**: `[RULER-SHADOW]` が「捨てた綴りなら別読みに
なる言及」を毎朝数える (07:57 実測 13 件)。0 件になった時の意味は二つ — 木から其の形の綴りが
消えたか、**物差しが巻き戻ったか** — 見分けは selftest T71/T72 が持つ。

**変異試験 (壊せば落ちるか・実測 2026-07-27 08:04)**: 物差しを捨てた綴りへ巻き戻すと
**T71 (枝つき幽霊の名指し) が exit 0 = 門が黙る / T71b (影の計器) / T72 (綴りの途中を拾わぬ)
の 3 本が赤**。T70 群は A/C/C? の三語と fail-closed (読めぬ台帳が在る間は「真に未登録」と
断ぜぬ) を縛る。

### 束で走らせる test は、視野計の分母から静かに落ちる (cmd_1408 / 2026-07-27・足軽六号)

**何が起きうるか**: 同伴 (companion) かどうかの判別は「entry の `test`/`mutate` に path が
そのまま現れるか」で決めている。ゆえに誰かが test を**束で走らせる**形
(`pytest tests/unit/` のようにディレクトリやワイルドカードで指す形) で書くと、
その test 本体は `paths` にしか現れず**同伴**と読まれ、視野計の分母から静かに落ちる。
落ちても画面には何も出ぬ。**「登録した」と「壊せば落ちる」を同じ物として数える**入口である。

**今の実態**: 6冊の台帳にこの形は **1 件も無い** (2026-07-27 09:32 実測・
本 gate に据えた後の再測でも 0 件)。**「今 無い」は「起こらぬ」ではない**ゆえ機械の側へ移した。

**門が何を刷るか**: `--coverage` が母数を毎回 名乗る。

```
  [束] 台帳 N 件を走査 / ★test を束 (ディレクトリ・ワイルドカード) で走らせる entry = 0 件★
```

0 件でも必ず刷る = 「0 件」と「そもそも数えておらぬ」を分ける。1 件以上なら
`[束/同伴] <entry id>: test が <綴り> を束で走らせる` と名指し、その entry の同伴も並べる。
**探し方**は runner (`pytest`/`bats` 等) より後ろの引数が dir か glob か、である。
runner の後ろに限るのは、引用符の中の綴りや `mkdir -p .venv/bin` の類を束と誤読せぬため。

**verdict は動かしておらぬ** = 名乗るに留める。理由は二つ — 今 0 件ゆえ門へ入れても
1bit も動かぬこと、そして**常に鳴る門は読まれなくなり必ず外される**こと。昇格は家老/殿の号令。

**変異試験 (両方向)**: T74a = 束を名乗る口を外せば赤 / T74b (負例) = test が本体を
名指しておれば**束と読まぬ** (常に鳴る門にせぬ)。片方だけでは、向きが逆の同じ病を作る。
