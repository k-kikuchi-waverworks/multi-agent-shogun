...
**作法** = 数える時は explicit path か `find ... | xargs grep` を使い、**未追跡は別勘定で数えよ**
（`git ls-files` は未追跡を含まぬゆえ**この盲点の代わりにはならぬ** — 下の撃ち直しで判った）。

### 7-1. ★撃ち直し (01:35)★ — 上の 14/13 は控えめ過ぎた。実寸は 559 対 15 である

家老の cmd_1399（本夜の「grep 0 件」を全て疑え）を受け、己の測り直しを行うた。**数は上書きせず、前の数も残す。**

| 問い「`def ` を含む .py は何本か」 | 答 |
|---|---|
| `find . -name '*.py' \| xargs grep -l` | **559 本** |
| `grep -rl --include='*.py' ... .` | **15 本** |
| （参考）`git ls-files '*.py'` | 16 本 |

⇒ **再帰 grep が見ておるのは、ほぼ「追跡下」と同じ範囲である。**
消えておる側の内訳 = `.venv` 433（third-party ゆえ落ちて構わぬ）/ **`plans/` 96** / **`scripts/` 11** / 他 4。
**とりわけ `scripts/` の 11 本は我らの【道具】である**（cmd_761/776/778/790 系・create_viewer_page・
generate_logo_cmd770・retired/dashboard-viewer）⇒ **「道具を全数当たった」と再帰 grep で名乗る者は、
此の 11 本を数えておらぬ。**

**己の断定も 1 件 偽であった** = 本 cmd の錨一意性を「repo 全域で **0 件**」と書いたが、
道具が 2 つとも同じ盲点を持っておった（再帰 grep は未追跡を見ず、`git ls-files` は未追跡を含まぬ ⇒
**当時 未追跡であった当の file が両方から落ちた**）。`find` で撃ち直すと **各 1 件**（本 file のみ）。
⇒ **結論（錨は一意）は生き残ったが、証拠は腐っておった。偶々 正しかっただけである。**
（台帳 MUT-1395-001/002 は mutate 冒頭で `s.count(old) == 1` を assert するゆえ、実走時は構造で保証される。）

**0 を報ずる時の作法** = canary（当たる筈の物を同じ道具で先に撃つ）に加え、**母数を並べよ**。
canary は「其の場所を見ておるか」を答えるが「何処を見ておらぬか」は答えぬ（本件の canary は
`scripts/` で当たったが、`plans/` 96 本の欠落は其れでは判らなんだ）。
⇒ **「0 件」でなく「N 本を見て 0 件」と書け。**
